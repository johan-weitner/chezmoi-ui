...
describe("selectors", () => {
	...
	it("should select app by key", () => {
		expect(selectors.selectAppByKey("app1")).toEqual(
			mockState.appCollection[0],
		);
	});

	it("should throw error if app by key not found", () => {
		expect(() => selectors.selectAppByKey("nonexistent")).toThrow(
			"App with key nonexistent not found",
		);
	});
	...
	it("should throw error if app by id not found", () => {
		expect(() => selectors.getAppById(999)).toThrow(
