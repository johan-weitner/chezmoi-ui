...
const createApplicationWithGroupsAndTags = async (payload) => {
	const { appData, appGroups, appTags } = payload;
	const { ...rest } = appData;
	const application = await prisma[APPLICATION].create({
		data: {
			...rest,
			appGroups: {
				connect: appGroups?.map((groupId) => {
					return { id: Number.parseInt(groupId, 10) };
				}),
			},
			appTags: {
				connect: appTags?.map((tagId) => {
					return { id: Number.parseInt(tagId, 10) };
				}),
			},
		},
		select: {
...
const updateApp = async (data) => {
	...
	try {
		const updatedApp = await prisma.application
			.update({
				where: { id: applicationId },
				data: {
					...rest,
					appGroups: {
						set: appGroups?.map((groupId) => {
							return { id: Number.parseInt(groupId, 10) };
						}),
					},
					appTags: {
						set: appTags?.map((tagId) => {
							return { id: Number.parseInt(tagId, 10) };
						}),
					},
				},
